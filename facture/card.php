...
//! CAUTION !!! Hookmanager will be no used here!!
...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
		...
		// We use 'I' by default for type of invoice
		$object->array_options['options_tipocomprobante'] = 'I';
		...
		if (GETPOST('type') == Facture::TYPE_CREDIT_NOTE) {
			...
			if (!$error) {
				...
				$object->array_options['options_tipocomprobante'] = 'E';
				$object->socid = GETPOST('socid', 'int');
...
if ($action == 'create') {
	...
	if ($soc->id > 0 && (!GETPOST('fac_rec', 'int') || !empty($invoice_predefined->frequency))) {
		...
		print '</td>';
		print '</tr>' . "\n";
		$object->array_options['options_regimenfiscalreceptor'] = $soc->array_options['options_regimenfiscal'];
	} else {
		print '<tr><td class="fieldrequired">' . $langs->trans('Customer') . '</td>';
		print '<td colspan="2">';
	...
	if (empty($reshook)) {
		// TODO: add function to get ddefault options on facture
		$object->array_options['options_metodopago'] = 'PUE';
		$object->array_options['options_formapago'] = '03';
		$object->array_options['options_usocfdi'] = 'G03';
		$object->array_options['options_exportacion'] = '01';
		if ($soc->id) {
			$_POST['options_regimenfiscalreceptor'] = $soc->array_options['options_regimenfiscal'];
		} else {
			$object->array_options['options_regimenfiscalreceptor'] = '601';
		}
